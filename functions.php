...
if ( ! function_exists( 'wordpress_theme_setup' ) ) :
	...
	function wordpress_theme_setup() {
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'wordpress_theme_custom_background_args',
				array(
...
function wordpress_theme_scripts() {
	wp_enqueue_style( 'wordpress-theme-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'wordpress-theme-style', 'rtl', 'replace' );

	wp_enqueue_script( 'wordpress-theme-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
